...
probe -- each saying "runs in the sandbox and on CI"; 37 as of #44). In the
root sandbox the non-root tests skip instead, saying "expected in the root
sandbox; CI runs it" (6 as of #18), plus one Windows-only test ("the read-only
attribute is a Windows shape"); as `nobody` the one root-only test skips
("needs root to plant another user's link"), and the Linux stage carries that
scenario on CI.
