...
| The same lines arrive twice | A delivery whose state could not be saved afterwards (`state not saved` in the log) -- the mail went out, then the next run re-sent it | Make the state directory the running user's -- owned by it, and so writable (`state directory ... is not writable`); the run refuses to send when it cannot save, once it knows |
